...
export class ReplayControls {
  ...
  mount(): void {
    ...
    backBtn.textContent = t('ui:replay.back');
    backBtn.addEventListener('click', () => this.onBack());
    topRow.appendChild(backBtn);

    // Skip to start
    const skipStartBtn = this.createControlBtn('|◄', t('ui:replay.skipToStart'));
    ...
    // Rewind 5s
    const rewBtn = this.createControlBtn('◄◄', t('ui:replay.rewind'));
    ...
    // Play/Pause
    this.playBtn = this.createControlBtn('▶', t('ui:replay.playPause'));
    ...
    // Forward 5s
    const fwdBtn = this.createControlBtn('►►', t('ui:replay.forward'));
    ...
    // Skip to end
    const skipEndBtn = this.createControlBtn('►|', t('ui:replay.skipToEnd'));
    ...
    // Speed button
    this.speedBtn = this.createControlBtn('1x', t('ui:replay.cycleSpeed'));
    this.speedBtn.style.cssText += 'margin-left:auto; min-width:40px; color:var(--accent);';
    ...
    info.textContent = t('ui:replay.matchInfo', {
      id: this.matchInfo.matchId,
      mode: this.matchInfo.gameMode.toUpperCase(),
      count: this.matchInfo.playerCount,
    });
  ...
  private createControlBtn(label: string, title: string): HTMLElement {
    ...
    btn.style.cssText = 'padding:4px 8px; font-size:12px; min-width:28px; text-align:center;';
