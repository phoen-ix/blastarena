...
export class UsersTab {
  ...
  private showDeleteModal(userId: number, username: string): void {
    const { overlay, content, close } = createModal({
      ariaLabel: t('admin:users.deleteModal.ariaLabel'),
  ...
  private showCreateUserModal(): void {
    const { overlay, content, close } = createModal({
      ariaLabel: t('admin:users.createModal.ariaLabel'),
      style: 'max-width:420px;',
      parent: document.getElementById('ui-overlay')!,
    });
    setHtml(
    ...
    overlay.querySelector('#cu-submit')!.addEventListener('click', async () => {
      const username = (overlay.querySelector('#cu-username') as HTMLInputElement).value.trim();
      const email = (overlay.querySelector('#cu-email') as HTMLInputElement).value.trim();
      const password = (overlay.querySelector('#cu-password') as HTMLInputElement).value;
      const role = (overlay.querySelector('#cu-role') as HTMLSelectElement).value;
      const errorEl = overlay.querySelector('#cu-error') as HTMLElement;
  ...
  private showResetPasswordModal(userId: number, username: string): void {
    const { overlay, content, close } = createModal({
      ariaLabel: t('admin:users.resetPasswordModal.ariaLabel'),
      style: 'max-width:420px;',
      parent: document.getElementById('ui-overlay')!,
    });
    setHtml(
    ...
    overlay.querySelector('#rp-submit')!.addEventListener('click', async () => {
      const password = (overlay.querySelector('#rp-password') as HTMLInputElement).value;
      const confirm = (overlay.querySelector('#rp-confirm') as HTMLInputElement).value;
      const errorEl = overlay.querySelector('#rp-error') as HTMLElement;
  ...
  private showCleanupModal(): void {
    const { overlay, content, close } = createModal({
      ariaLabel: t('admin:users.cleanup.title'),
    ...
    const renderModal = () => {
      ...
      // Preview
      overlay.querySelector('#cleanup-preview')?.addEventListener('click', async () => {
        const errorEl = overlay.querySelector('#cleanup-error') as HTMLElement;
        const needsDaysNow = selectedType !== 'deactivated';
        const daysInput = overlay.querySelector('#cleanup-days') as HTMLInputElement | null;
        ...
        try {
          ...
          const newDaysInput = overlay.querySelector('#cleanup-days') as HTMLInputElement | null;
      ...
      const confirmInput = overlay.querySelector(
        '#cleanup-confirm-input',
      ) as HTMLInputElement | null;
      const executeBtn = overlay.querySelector('#cleanup-execute') as HTMLButtonElement | null;
      ...
      // Close handler
      overlay.querySelector('#cleanup-cancel')?.addEventListener('click', close);
    };
