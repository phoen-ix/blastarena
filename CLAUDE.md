...
- **Local co-op**: Same client, one socket sends both players' inputs. P2 can be guest (negative temp ID, no DB writes) or logged-in. Setup modal (`LocalCoopModal.ts`) lets players assign control presets and camera mode before starting; config persisted in localStorage (`blast-arena-local-coop-controls`) and `localCoopConfig` registry (survives retry/next-level without re-showing modal). 5 control presets: WASD+Space/E, Arrows+Enter/Shift, Numpad 8462/+/-, Gamepad 1, Gamepad 2 — each assignable to either player, conflict-validated. `LocalCoopInput.ts` handles configurable dual-player input with per-player just-pressed tracking. 3 camera modes: Shared (auto-zoom midpoint), Split Horizontal (top/bottom), Split Vertical (left/right). Split-screen uses `p2Camera` via `this.cameras.add()` with auto-zoom to fill each viewport; per-axis camera lock prevents map misalignment when zoomed map fits within viewport. Off-screen partner arrows (cyan triangles at viewport edge) appear in split-screen when the other player is not visible. Divider: 6px dark outline + 2px white inner line.
...
- **Registry flags**: `campaignMode`, `campaignCoopMode`, `localCoopMode`, `localCoopConfig` control frontend behavior (camera, input, HUD).
