...
- **Language switcher**: Flag dropdown on auth screen (top-right of form, `AuthUI.renderLanguagePicker()`); `<select>` in Settings > Preferences. Both call `i18n.changeLanguage()` which dispatches `language-changed` window event. SettingsUI listens and re-renders on language change
