...
- **HUD**: DOM-based overlay in HUDScene.ts. Timer counts up for campaign levels with no time limit (`roundTime >= 99999`), counts down otherwise. In-game minimap (Canvas2D, 140px max, bottom-right) shows tiles, players, bombs, explosions, zone/hill. Toggleable via settings, throttled to every 4 ticks. Minimap maintains its own `minimapTiles` copy (populated from initial full state, updated via `tileDiffs`) since tick states omit full tile grid. Kill feed shows cause icons (bomb, zone, hazard type, self, disconnect). Death banner for local player with killer info
