...
- Custom maps: `custom_maps` DB table (migration 026). `customMapId` on `MatchConfig`, `customMapName` on `Room`/`RoomListItem`. Backend loads map on `room:start`, passes to `GameStateManager` via existing `config.customMap ?? generateMap()`. CRUD service in `backend/src/services/custom-maps.ts`, routes in `backend/src/routes/custom-maps.ts`. Shared validation in `shared/src/utils/mapValidation.ts` (odd dims 9-51, border walls, 2-8 spawns, allowed tiles, teleporter pairing)
...
- **Rendering**: GameScene delegates to renderer classes in `frontend/src/game/`. `activeMoveAnim` Set on PlayerSprite prevents tween stacking. All sprites procedurally generated in `BootScene.generateTextures()` — no external image assets. Conveyor belts use Phaser spritesheet animations (4 frames × 4 directions, `conveyor_{dir}_anim` keys, 8fps looping) — the only tile type using `sprite.play()`; respects `settings.animations` flag in gameplay, always-on in editor. Power-up icons are procedural Canvas2D drawings (`powerUpIcons.ts`) shared between BootScene (in-game textures) and `powerUpCanvas.ts` (UI previews) — no emoji `fillText`, guaranteed cross-platform. Tile previews in DOM UI (Help panel, Level Editor docs) use `tileCanvas.ts` — replicates BootScene procedural tile textures as Canvas2D, supports all 27 tile types with optional `CampaignThemePalette` parameter for themed rendering. Map previews use `mapPreview.ts` — renders `TileType[][]` as a pixel-art minimap canvas (color-coded squares, dynamic tile size based on map dimensions, optional theme palette). `mapPreviewCache.ts` lazy-loads and caches tile data from API with in-flight request deduplication. `MapEventRenderer` handles meteor warnings (pulsing crosshair + exclamation + growing shadow), impact animations (falling meteor sprite + flash + particles + screen shake), and UFO abduction (procedural saucer with dome, pulsing tractor beam triangle, green flash on abduction). Enemy textures on-demand via `EnemyTextureGenerator`. Enemy sprite addons: 6 body shapes × 4 eye styles × 8 visual addons (teeth, horns, tail, aura, crown, scar, wings) + accessory dropdown (none/bow_tie/monocle/bandana). Crown and horns are mutually exclusive. All addons rendered in both Phaser Graphics (directional, 48x48) and Canvas2D (preview). New fields on `EnemySpriteConfig` are optional with `?? false` / `?? 'none'` fallbacks for backward compatibility
...
- Editor supports dual mode: `editorMode: 'campaign' | 'custom_map'` (defaults to `'campaign'`). Custom map mode hides campaign-only tools and uses `/maps` API for save/load. See Custom Maps section
...
## Custom Maps
User-created maps for multiplayer rooms. Reuses the campaign `LevelEditorScene` with `editorMode: 'custom_map'` (set via Phaser registry) which hides campaign-only tools (enemies, puzzles, hazards, power-ups, exit/goal tiles). Allowed multiplayer tile types: `empty`, `wall`, `destructible`, `spawn`, `teleporter_a/b`, `conveyor_up/down/left/right`.
- **DB**: `custom_maps` table (migration 026) with tiles/spawn_points as JSON, `created_by` FK to users, `is_published` flag, `play_count` counter
- **API**: `backend/src/routes/custom-maps.ts` — CRUD at `/maps/mine`, `/maps/published`, `/maps/:id`. Owner-only update/delete. Zod + `validateCustomMap()` server-side
- **Room integration**: `MatchConfig.customMapId` validated in `room:create` (map must exist). `MatchConfig.selectedHazardTiles` and `MatchConfig.selectedMapEvents` specify which hazard/event types are active (all enabled by default). On `room:start`, map loaded from DB, built into `GameMap { width, height, tiles, spawnPoints, seed: 0 }`, passed through `RoomManager.createGame()` → `GameRoom` → `GameStateManager` config. `GameConfig.hazardTiles` and `GameConfig.enabledMapEvents` carry the selected types. `customMapName` stored on Room for display in room list
- **Editor**: `LevelEditorScene` reads `editorMode` and `customMapId` from registry in `create()`. `saveCustomMap()` POSTs/PUTs to `/maps` API. `loadData()` fetches from `/maps/:id` for existing maps. Settings panel shows Name/Description/Published for custom maps, hides campaign-specific fields (Lives, Time Limit, Par Time, Win Condition)
- **Frontend views**: `MapsView` in sidebar "My Maps" — table listing with Edit/Publish/Delete. Room creation (`CreateRoomView` + `CreateRoomModal`) fetches `/maps/mine` + `/maps/published`, shows map dropdown with optgroups, disables map size/wall density when custom map selected. Hazard Tiles and Map Events toggles show sub-panels with option chips for selecting specific types (all checked by default)
- **Map previews**: Campaign level selection and room creation (both `CreateRoomView` and `CreateRoomModal`) show a pixel-art minimap canvas when a level/custom map is selected. Campaign previews use the world's `CampaignThemePalette` for themed coloring. Tile data lazy-loaded via `GET /campaign/levels/:id` or `GET /maps/:id`, cached in `mapPreviewCache.ts`. Stale-guard checks prevent inserting a canvas for a deselected level/map
...
- Shield has no time limit — lasts until consumed. After break, 10 ticks invulnerability. Extra pickups consumed but don't stack. Shield absorbs all damage sources: explosions, enemy contact, quicksand, spikes
...
- **Dynamic map events** (optional, individually selectable via `selectedMapEvents` on `MatchConfig`): Meteor strikes every 30-45s (40-tick/2s warning with crosshair reticle, exclamation mark, growing shadow; impact triggers falling meteor animation, flash, debris/fire/spark particles, screen shake). `ownerId: -999` for system-owned meteor explosions (2-tile blast radius). Power-up rain every 60s. Wall collapse every 45-60s (40-tick warning, destroys 3x3 destructible area). Freeze wave every 50-70s (20-tick warning, converts row/column to ice for 120 ticks via `frozenTiles` Map, then reverts). Bomb surge every 40-55s (instant, all bombs lose 20 fuse ticks, red screen pulse + shake). UFO abduction every 35-50s (40-tick/2s warning with tractor beam on random alive player; teleports target to random walkable tile with brief invulnerability; `targetPlayerId` on `MapEvent`). `enabledMapEventTypes` Set on GameState filters which event types can trigger. Shared types: `MapEventType`, `MAP_EVENT_TYPES` array; `MapEvent` interface includes `'ufo_abduction'` type and `targetPlayerId` field. `MapEventRenderer` in `frontend/src/game/MapEventRenderer.ts`
- **Hazard tiles** (optional, individually selectable via `selectedHazardTiles` on `MatchConfig`): Teleporter pairs (A↔B, seeded-RNG destination selection, works for players and campaign enemies), conveyor belts (auto-push players, bombs, and campaign enemies in direction when movement cooldown ready, chain into teleporters, animated moving stripes via 4-frame Phaser spritesheet animation at 8fps). Bombs on conveyors use `MOVE_COOLDOWN_BASE` tick cooldown between pushes; kicked bombs (already sliding) ignore conveyors. Conveyor belts also push campaign enemies (except `canPassWalls` enemies) — processed before AI so conveyor consumes the move cooldown and AI skips that tick. Additional hazard tiles (work in both multiplayer and campaign): vine (slows movement), quicksand (slows + kills after `QUICKSAND_KILL_TICKS`), ice (sliding momentum), lava (instant kill, auto-detonates adjacent bombs), mud (slows movement), spikes/spikes_active (cycling damage via `SPIKE_SAFE_TICKS`/`SPIKE_CYCLE_TICKS`), dark_rift (teleports to random empty tile). In multiplayer, `generateMap()` places clustered hazard tiles on ~4% of empty tiles, filtered by the `hazardTiles` array in `GameConfig`. `GameState.ts` has full hazard processing: quicksand timers, ice sliding momentum, lava bomb detonation, spike cycling, dark rift teleport, vine/quicksand/mud movement slowdown. Slowing tiles use `movedThisTick` flag on Player/Enemy to apply slowdown only once per move — not every tick. Shared types: `HazardTileType`, `HAZARD_TILE_TYPES` array. Hazard tile types, constants, and theme mappings in `shared/src/utils/hazard.ts` and `shared/src/constants/campaignThemes.ts`
...
- `room:create` socket event validates `MatchConfig` via Zod schema (game mode, map dimensions, player count, power-ups, selected hazard tiles, selected map events, etc.)
...
- **Adding a language**: Create `{lng}/` dirs in all three locale paths, translate all JSON files (must have 1:1 key parity with `en/`), add language code to `supportedLngs` in both `frontend/src/i18n/index.ts` and `backend/src/i18n/index.ts`, add `<option>` in `SettingsUI.ts` language selector, add entry in `AuthUI.ts` language picker
- **Language switcher**: Flag dropdown on auth screen (top-right of form, `AuthUI.renderLanguagePicker()`); `<select>` in Settings > Preferences. Both call `i18n.changeLanguage()` which dispatches `language-changed` window event
- **Language-changed re-render**: Persistent UI components (`SettingsUI`, `LobbyUI`, `AdminUI`, `HelpUI`) listen for `language-changed` window event and re-render. Components with constructor-captured tab labels refresh them in `render()` before rebuilding HTML. `SocketClient` also listens to keep `socket.auth.locale` in sync for reconnections
