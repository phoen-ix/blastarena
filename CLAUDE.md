...
- **Room integration**: `MatchConfig.customMapId` validated in `room:create` (map must exist). On `room:start`, map loaded from DB, built into `GameMap { width, height, tiles, spawnPoints, seed: 0 }`, passed through `RoomManager.createGame()` → `GameRoom` → `GameStateManager` config. `customMapName` stored on Room for display in room list
...
- **Frontend views**: `MapsView` in sidebar "My Maps" — table listing with Edit/Publish/Delete. Room creation (`CreateRoomView` + `CreateRoomModal`) fetches `/maps/mine` + `/maps/published`, shows map dropdown with optgroups, disables map size/wall density when custom map selected
