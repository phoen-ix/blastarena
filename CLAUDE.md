...
- **Visual**: Smaller sprite (configurable 40-80% via `setDisplaySize()`, enforced every frame to survive texture swaps and tweens), pulsing glow aura (`Phaser.GameObjects.Graphics` at depth 8, slower oscillation than shield). `[BUDDY]` tag in HUD player list. `PlayerSpriteRenderer.update()` auto-detects buddy from `PlayerState.isBuddy` as fallback if `setBuddyPlayer()` wasn't called.
