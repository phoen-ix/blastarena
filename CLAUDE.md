...
- **HUD**: DOM-based overlay in HUDScene.ts. Timer counts up for campaign levels with no time limit (`roundTime >= 99999`), counts down otherwise. In-game minimap (Canvas2D, 140px max, bottom-right) shows tiles, players, bombs, explosions, zone/hill. Toggleable via settings, throttled to every 4 ticks. Minimap maintains its own `minimapTiles` copy (seeded from `initialGameState` registry in `create()`, updated via `tileDiffs`) since tick states omit full tile grid — GameScene emits `stateUpdate` with full tiles during its `create()` before HUDScene registers its listener, so HUDScene seeds directly from registry. Kill feed shows cause icons (bomb, zone, hazard type, self, disconnect). Death banner for local player with killer info
...
- **Toroidal map**: Wrapping 51x41 grid (default, no border walls). Coordinates wrap via `wrapX`/`wrapY` from `shared/src/utils/wrap.ts`. CollisionSystem wraps coordinates, `Map.ts` generates borderless maps with distributed spawns, `getExplosionCells` wraps, BotAI uses `wrappedManhattanDistance`. For odd-dimension wrapping maps, the last even col/row is skipped in wall grid generation to avoid double-walls at the wrapping seam. `EffectSystem.wrappingMapSize` enables wrapped distance for explosion audio volume and screen shake — set by GameScene alongside renderer wrapping
...
- **TOTP Two-Factor Authentication**: Optional per-user 2FA via authenticator apps. `TOTP_ENCRYPTION_KEY` env var (AES-256-GCM) encrypts secrets at rest. Two-step login: `POST /auth/login` returns `TotpChallengeResponse` (5-min JWT with `purpose: 'totp-challenge'`) when TOTP enabled; `POST /auth/verify-totp` exchanges challenge token + 6-digit code for normal `AuthResponse`. Setup flow: `POST /user/totp/setup` generates secret + QR data URI + 10 bcrypt-hashed backup codes (stored in `totp_backup_codes` JSON column); `POST /user/totp/confirm` validates code before enabling. Disable requires password + TOTP code via `POST /user/totp/disable`. `PublicUser.twoFactorEnabled` field. Backup codes normalized lowercase, removed from array on use. Frontend: AuthUI `'totp'` mode for login verification; SettingsUI 2FA section (between Password and Danger Zone) with setup modal (QR + plaintext secret + backup codes) and disable modal. Migration 036. Uses `otpauth` + `qrcode` packages
